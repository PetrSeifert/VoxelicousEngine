...
{
	SwapChain::SwapChain(Device& deviceRef, VkExtent2D extent, VkSurfaceKHR surface)
		: m_Device{ deviceRef }, m_WindowExtent{ extent }
	{
		Init(surface);
	}

	SwapChain::SwapChain(Device& deviceRef, VkExtent2D extent, VkSurfaceKHR surface, std::shared_ptr<SwapChain> previous)
	...
	{
		SwapChainSupportDetails swapChainSupport = m_Device.GetSwapChainSupport(surface);
		...
		QueueFamilyIndices indices = m_Device.FindPhysicalQueueFamilies(surface);
