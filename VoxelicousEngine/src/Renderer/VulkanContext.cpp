...
namespace VoxelicousEngine {
...
#if defined(VE_DEBUG) || defined(VE_RELEASE)
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDebugReportCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
	{
		...
		VE_CORE_WARN_TAG("Renderer", "VulkanDebugCallback:\n  Object Type: {0}\n  Message: {1}", objectType, pMessage);
	...
	static bool CheckDriverAPIVersionSupport(uint32_t minimumSupportedVersion)
	{
		...
		{
			VE_CORE_FATAL("Incompatible Vulkan driver version!");
			VE_CORE_FATAL("  You have {}.{}.{}", VK_API_VERSION_MAJOR(instanceVersion), VK_API_VERSION_MINOR(instanceVersion), VK_API_VERSION_PATCH(instanceVersion));
			VE_CORE_FATAL("  You need at least {}.{}.{}", VK_API_VERSION_MAJOR(minimumSupportedVersion), VK_API_VERSION_MINOR(minimumSupportedVersion), VK_API_VERSION_PATCH(minimumSupportedVersion));
	...
	void VulkanContext::Init()
	{
		...
		{
#ifdef VE_PLATFORM_WINDOWS
			MessageBox(nullptr, L"Incompatible Vulkan driver version.\nUpdate your GPU drivers!", L"VoxelicousEngine Error", MB_OK | MB_ICONERROR);
#else
			VE_CORE_ERROR("Incompatible Vulkan driver version.\nUpdate your GPU drivers!");
#endif
			VE_CORE_VERIFY(false);
		}
		...
		appInfo.pApplicationName = "VoxelicousEngine";
		appInfo.pEngineName = "VoxelicousEngine";
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_xcb_surface"
#endif
		std::vector instanceExtensions = { VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_WIN32_SURFACE_EXTENSION_NAME };
		...
		if (s_Validation)
		{
			...
			VE_CORE_INFO_TAG("Renderer", "Vulkan Instance Layers:");
			for (const VkLayerProperties& layer : instanceLayerProperties)
			{
				VE_CORE_INFO_TAG("Renderer", "  {0}", layer.layerName);
			...
			{
				VE_CORE_ERROR_TAG("Renderer", "Validation layer VK_LAYER_KHRONOS_validation not present, validation is disabled");
			}
		}
		...
		if (s_Validation)
		{
			...
			VE_CORE_ASSERT(vkCreateDebugReportCallbackEXT != NULL, "");
			...
			VE_CORE_ASSERT(vkCreateDebugUtilsMessengerEXT != NULL, "");
		...
		m_Device = Ref<Device>::Create(m_PhysicalDevice, enabledFeatures);
